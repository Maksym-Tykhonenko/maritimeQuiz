...
import {topick3} from '../../data/topick3';
...
const Lvls3 = ({navigation, route}) => {
  ...
  const [usedHintsForCurrentQuestion, setUsedHintsForCurrentQuestion] =
    useState(false);
  //console.log('topickLength==>', topick3.length);
  ...
  useEffect(() => {
    if (currentQuestionIndex < 10) {
      const currentQuestion = topick3[currentQuestionIndex];
      setOptions(
  ...
  return (
    <View
      ...
      {currentQuestionIndex >= topick3.length ? (
        <View
          style={{
            flex: 1,
            alignItems: 'center',
        ...
        <View style={{paddingHorizontal: 10}}>
          ...
          <ScrollView>
            <Image
              source={require('../../assets/Stage2/T3.png')}
