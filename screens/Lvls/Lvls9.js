...
import {topick9} from '../../data/topick9';
...
const Lvls9 = ({navigation, route}) => {
  ...
  const [usedHintsForCurrentQuestion, setUsedHintsForCurrentQuestion] =
    useState(false);
  //console.log('topickLength==>', topick9.length);
  ...
  useEffect(() => {
    if (currentQuestionIndex < 10) {
      const currentQuestion = topick9[currentQuestionIndex];
      setOptions(
  ...
  return (
    <View
      ...
      {currentQuestionIndex >= topick9.length ? (
        <View
          style={{
            flex: 1,
            alignItems: 'center',
        ...
        <View style={{paddingHorizontal: 10}}>
          ...
          <ScrollView>
            <Image
              source={require('../../assets/Stage2/T9.png')}
