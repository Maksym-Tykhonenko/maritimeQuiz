...
import {topick6} from '../../data/topick6';
...
const Lvls6 = ({navigation, route}) => {
  ...
  const [usedHintsForCurrentQuestion, setUsedHintsForCurrentQuestion] =
    useState(false);
  //console.log('topickLength==>', topick6.length);
  ...
  useEffect(() => {
    if (currentQuestionIndex < 10) {
      const currentQuestion = topick6[currentQuestionIndex];
      setOptions(
  ...
  return (
    <View
      ...
      {currentQuestionIndex >= topick6.length ? (
        <View
          style={{
            flex: 1,
            alignItems: 'center',
        ...
        <View style={{paddingHorizontal: 10}}>
          ...
          <ScrollView>
            <Image
              source={require('../../assets/Stage2/T6.png')}
